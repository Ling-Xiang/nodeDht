...
DHTSpider.prototype.onAnnouncePeerRequest = function(msg, rinfo) {
    ...
    var str =  util.format("INSERT INTO dht(hash,ip)  VALUES ('%s','%s')", infohash.toString("hex"), rinfo.address);
    ...
    // });
    // conn.end();
    mysql.query(str, function (error, res) {
     console.log("then");
    });
...
　　　  console.log("执行去重任务");
    ...
    //console.log("magnet:?xt=urn:btih:%s from %s:%s", infohash.toString("hex"), rinfo.address, rinfo.port);
       var util = require('util');
       var str =  "DELETE FROM dht WHERE id NOT IN ( SELECT * FROM (( SELECT min(id) FROM dht GROUP BY (HASH)) AS tmptable ))";
